...
bool ModulePlayer::Start()
{
	...
	player_col = App->collision->AddCollider({ 0, 0, 32, 15 }, COLLIDER_PLAYER);
...
update_status ModulePlayer::Update()
{
	...
	if (player_col->CheckCollision(App->scene_space->bottom_wall->rect))OnCollision();
	if (player_col->CheckCollision(App->scene_space->upper_wall->rect))OnCollision();
	if (player_col->CheckCollision(App->scene_space->ground->rect))OnCollision();
